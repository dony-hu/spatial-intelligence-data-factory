...
def test_run_prompt_model_can_be_loaded_from_config_file(tmp_path: Path, monkeypatch) -> None:
    config_path = tmp_path / "llm_api.json"
    config_path.write_text('{"opencode_model":"alibaba-cn/qwen-max"}', encoding="utf-8")
    monkeypatch.delenv("OPENCODE_MODEL", raising=False)
    runtime = OpenCodeRuntime(config_path=str(config_path))
    captured = {}
    ...
    monkeypatch.setattr("subprocess.run", _fake_run)
    runtime._run_opencode_prompt("hello")
    assert captured["cmd"][-2:] == ["--model", "alibaba-cn/qwen-max"]
