...
def clean_db(db_engine):
    ...
    if db_engine:
        ...
        with db_engine.begin() as conn:
            conn.execute(text("TRUNCATE TABLE governance.task_run CASCADE"))
            conn.execute(text("TRUNCATE TABLE governance.raw_record CASCADE"))
            conn.execute(text("TRUNCATE TABLE governance.canonical_record CASCADE"))
            conn.execute(text("TRUNCATE TABLE governance.review CASCADE"))
            conn.execute(text("TRUNCATE TABLE audit.api_audit_log CASCADE"))
    yield
...
def test_happy_path_clean_address(client, clean_db, db_engine):
    ...
    if db_engine:
        with db_engine.connect() as conn:
            ...
            # Check governance.canonical_record
...
def test_manual_review_flow(client, clean_db, db_engine):
    ...
    # Depending on the runtime stub/pipeline, verify confidence/strategy
    # For now, we assume the pipeline returns lower confidence for this input
    # If using runtime stub, it might return fixed values. 
    ...
    if db_engine:
        with db_engine.connect() as conn:
            review_row = conn.execute(
                text("SELECT final_canon_text, review_status FROM governance.review WHERE review_id = :review_id"),
