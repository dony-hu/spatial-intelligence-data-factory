...
## 12. 云上建设与专网迁移技术选型（新增强约束）

### 12.1 设计约束（硬约束）

1. 平台必须可在专网环境快速移植部署，禁止绑定互联网主流 PaaS 能力。
2. 云上阶段仅依赖基础 IaaS 能力：计算、网络、块存储、对象存储、负载均衡。
3. 应用与平台组件必须通过 IaC 和容器化实现同构交付，禁止控制台手工配置作为唯一来源。
4. 大模型能力必须支持并行适配：Qwen、GLM、腾讯混元，且业务侧使用统一协议接入。

### 12.2 基础设施选型（云上/专网同构）

1. 计算与编排
- 计算资源：VM/ECS（专网可替换为虚拟化资源池）。
- 编排平台：Kubernetes（kubeadm + containerd），小规模场景可选 K3s。
- 网络：VPC/子网/安全组 + L4/L7 负载均衡。

2. 存储
- 对象存储：云上可用云 OSS，平台接口统一 S3 兼容；专网默认 MinIO。
- 持久卷：Kubernetes CSI + 块存储；专网替换为 Ceph RBD 或等价方案。
- 备份恢复：Velero + 对象存储，支持跨环境恢复演练。

3. 数据库与中间件
- 关系库：PostgreSQL（主库）。
- 缓存：Redis（自建哨兵/集群）。
- 消息队列：RabbitMQ（通用）或 Kafka（高吞吐场景）。
- 检索（可选）：OpenSearch（避免云厂商专有检索绑定）。

4. 制品与发布
- 镜像仓库：Harbor。
- 发布体系：Helm + Argo CD（GitOps）。
- 基础自动化：Terraform + Ansible + Helm（禁止手工漂移）。

### 12.3 大模型兼容架构（Qwen / GLM / 腾讯混元）

1. 统一模型接入层（Model Gateway）
- 在平台内建设统一模型网关，业务只调用统一 API（OpenAI 兼容协议）。
- 网关内置供应商适配器：QwenAdapter、GLMAdapter、HunyuanAdapter。
- 统一能力面：`chat/completions`、`embeddings`、`rerank`、`tool calling`。

2. 双轨部署策略（云上先行 + 专网落地）
- 云上阶段：调用各厂商 API（Qwen、GLM、腾讯混元）。
- 专网阶段：切换至本地推理服务（vLLM/SGLang + 本地模型权重）。
- 切换原则：仅调整模型网关后端路由，不改业务代码和上层契约。

3. RAG 与检索增强（按需启用）
- 向量库：Milvus（首选）或 PostgreSQL + pgvector。
- Embedding/Rerank 服务：优先本地化部署，避免外部依赖。

### 12.4 接口与治理标准（必须统一）

1. 协议标准：内部统一 OpenAI 兼容协议，禁止业务直接绑定单一厂商 SDK。
2. 路由标准：按任务类型、时延、成本、可用性进行模型路由与降级。
3. 稳定性标准：统一超时、重试、熔断、限流、幂等策略与错误码。
4. 安全合规标准：提示词、上下文、日志脱敏与审计策略统一执行。
5. 离线交付标准：镜像、模型权重、依赖包、Helm Chart 必须具备离线分发清单。

### 12.5 实施建议（近期）

1. 建设最小同构环境（K8s + PostgreSQL + MinIO + Harbor + 监控）并完成一次专网模拟迁移演练。
2. 优先落地模型网关 MVP，先打通 Qwen/GLM/腾讯混元三路适配，再接业务场景。
3. 形成《模型网关接口规范》与《离线部署手册》，作为后续子项目统一准入门槛。
