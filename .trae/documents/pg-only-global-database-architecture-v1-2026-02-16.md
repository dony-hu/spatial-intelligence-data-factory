...
**设计变更**：
基于未来不同项目数据可能由不同数据库实例管理的预期，我们**不采用**“单表+Namespace字段”的逻辑隔离方案作为唯一的隔离手段。

**新隔离策略**：
1. **Schema/Instance 级隔离**：
   - 不同的项目/租户应优先通过独立的 **Schema** 或 **Database Instance** 进行物理隔离。
   - 这允许不同项目使用完全独立的资源、备份策略和权限控制。
2. **表内 `namespace_id` 的角色**：
   - 表内的 `namespace_id` 字段保留，但其语义降级为“逻辑分组”或“数据来源标识”，而非安全隔离边界。
   - 例如，在一个 `shanghai_project` 的 Schema 中，`namespace_id` 可能用于区分 `gov_source` 和 `internet_source`，而不是区分 `project_A` 和 `project_B`。
3. **复合主键的必要性**：
   - 即使采用 Schema 隔离，保留 `(namespace_id, snapshot_id)` 复合主键依然合理。它确保了在同一个物理实例中，数据的唯一性约束是完备的，且支持未来可能的聚合查询场景。
