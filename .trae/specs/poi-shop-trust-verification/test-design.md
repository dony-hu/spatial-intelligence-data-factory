...
### Test Case 2.4: 系统复位与重建测试（关键验收）
**目标**: 验证清空 workpackages 目录后，通过测试系统与工厂 CLI 对话，完成地址治理（标准化、去重、验真）工作包的重建
**前置条件**: 无
**步骤**:
1. 清空 workpackages 目录下的内容（系统复位）
2. 测试系统与工厂 CLI 对话
3. 通过 converse() 对话确定 2~3 家外部数据源
4. 通过 converse() 提供 API Key
5. 工厂 Agent 生成完整地址治理工作包（包含标准化、去重、验真）
**预期结果**:
- 系统复位成功
- 2~3 家外部数据源确定并存储
- API Key 存储到可信数据 HUB
