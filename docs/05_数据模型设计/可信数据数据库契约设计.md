...
| `trust_meta.source_schedule` | 来源调度策略 | 可信数据管理模块 | 管理台、调度链路 | 正式调度配置入口 |
| `trust_meta.source_snapshot` | 快照版本 | 导入链路 | 管理台、Agent、回放 | 正式快照入口 |
| `trust_meta.snapshot_quality_report` | 快照质量报告 | 导入链路 / 校验链路 | 管理台、发布流程、回放 | 正式质量评估入口 |
| `trust_meta.snapshot_diff_report` | 快照差异报告 | 导入链路 / 对账链路 | 管理台、发布流程、回放 | 正式差异评估入口 |
...
| `trust_meta.validation_replay_run` | 回放验证结果 | 校验链路 | 管理台、回放、验收 | 正式 replay 口径 |
| `trust_meta.audit_event` | 可信数据域审计 | 管理模块 / 发布流程 / 校验链路 | 管理台、审计工具 | 正式域内审计口径 |
...
| 写入方 | 允许写入 |
|---|---|
| 可信数据管理模块 | `trust_meta.source_registry`、`trust_meta.source_schedule`、`trust_meta.capability_registry`、`trust_meta.audit_event` |
| 导入链路 | `trust_meta.source_snapshot`、`trust_meta.snapshot_quality_report`、`trust_meta.snapshot_diff_report`、`trust_meta.validation_replay_run`、`trust_data.*` |
...
| 读取方 | 正式读取口径 |
|---|---|
| Factory Agent | `trust_meta.capability_registry`、`trust_meta.active_release`、`trust_meta.source_snapshot` 摘要 |
| Runtime / Bundle | `trust_data.*`、必要的能力元数据、激活版本摘要 |
...
| `trust_meta.source_schedule` | 可信数据管理模块 | 调度管理接口 | 调度策略归 `trust_meta`，不应散落到脚本配置 |
| `trust_meta.source_snapshot` | 导入链路 / 发布流程 | 快照与激活接口 | Agent 只读摘要，不直写 |
| `trust_meta.snapshot_quality_report` / `snapshot_diff_report` | 校验与发布链路 | 质量 / 差异接口 | 发布门禁应读取这里的正式评估结果 |
...
| `trust_meta.validation_replay_run` | 校验回放链路 | replay 接口 | 回放结果必须结构化落库，不只写日志 |
| `trust_meta.audit_event` | 管理模块 / 发布流程 / 校验链路 | 审计接口 | 可信数据域内关键动作要有独立审计面 |
...
6. `trust_meta` 的正式表组至少包括：
   - `source_registry / source_schedule / source_snapshot`
   - `snapshot_quality_report / snapshot_diff_report / validation_replay_run`
   - `active_release / capability_registry / audit_event`
7. 可信数据正式接口默认只对应两类数据库域：
   - `trust_meta` 元数据、回放与审计
   - `trust_data` 标准查询数据
8. 任何新 API 设计若仍引用 `trust_db.*`，应视为未收敛完成
