# 研发主题 - 系统可观测性与 PG 库一体化建设（2026-02-27）
...
1. 当前现行架构基线以 `docs/02_总体架构/架构索引.md` 及其列出的真相源文档为准。
2. `archive/docs/architecture/architecture-unified-pg-multi-schema-v1-2026-02-27.md` 降级为 PG 与 schema 演进的历史专题参考。

## 2. In Scope

1. 统一可观测事件模型（trace/event/metric/alert）。
2. 统一 PG 表结构与索引（含 `governance/runtime/control_plane/trust_meta/trust_data/audit` 查询对齐）。
