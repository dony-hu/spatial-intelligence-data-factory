...
- [ ] 任务 1：盘点共享契约与红区对象（AC: 1, 3）
  - [ ] 子任务 1.1：从正式规范、AGENTS 和主题设计中提取共享契约对象全集
  - [ ] 子任务 1.2：区分“唯一 owner 串行修改”和“先契约 PR 后下游实现”两种处理路径
  - [ ] 子任务 1.3：补充当前规范未显式列出、但已在冲刺计划中列为红区的对象
- [ ] 任务 2：固化 owner 与审批路径（AC: 2, 4）
  - [ ] 子任务 2.1：为每个共享契约对象指定 owner 角色
  - [ ] 子任务 2.2：定义契约 PR、实现 PR、回归确认三者的责任边界
  - [ ] 子任务 2.3：明确“每日唯一共享契约 PR”与集成值班确认规则
- [ ] 任务 3：回填 Epic13 过程文档（AC: 3, 4, 6）
  - [ ] 子任务 3.1：将共享契约冻结清单回填到 Epic13 主题目录中的正式过程文档
  - [ ] 子任务 3.2：确保后续 `PAR-S4`、`PAR-S5` 可直接引用本 Story 的结论
  - [ ] 子任务 3.3：如影响 Ring1 正式规范，明确需要同步更新的正式文档入口
- [ ] 任务 4：准备验收检查单（AC: 5, 6）
  - [ ] 子任务 4.1：给出最小人工验收清单
  - [ ] 子任务 4.2：列出下游消费方需要回归的点
  - [ ] 子任务 4.3：标记仍受 Linear 外部 blocker 影响的环节
