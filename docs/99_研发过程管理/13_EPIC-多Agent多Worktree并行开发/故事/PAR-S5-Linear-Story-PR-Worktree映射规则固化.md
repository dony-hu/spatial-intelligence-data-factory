...
- 允许依赖：
  ...
  - `docs/99_研发过程管理/13_EPIC-多Agent多Worktree并行开发/映射规则与PR必填字段.md`
- 禁止依赖：
...
- [x] 任务 1：固化最小映射模型（AC: 1, 2）
  - [x] 子任务 1.1：定义一条并行工作流的最小绑定对象集合
  - [x] 子任务 1.2：说明缺失映射对象时的禁止动作
  - [x] 子任务 1.3：区分 Epic 级后续项与 Story 级已完成项
- [x] 任务 2：固化命名与登记规则（AC: 1, 3）
  - [x] 子任务 2.1：定义 branch、Worktree 和 Story 的映射写法
  - [x] 子任务 2.2：定义 PR 标题 / 描述的最小必填字段
  - [x] 子任务 2.3：定义 owner 与受影响下游的登记口径
- [x] 任务 3：固化状态跟踪口径（AC: 4, 6）
  - [x] 子任务 3.1：定义 `ready-for-dev / in-progress / done / blocked / follow-up` 的使用边界
  - [x] 子任务 3.2：说明 Story 状态、验收结论和 Epic 外部后续项的关系
  - [x] 子任务 3.3：说明哪些状态可被集成值班和 `lane-06` gate 直接消费
- [x] 任务 4：准备验收检查单（AC: 5, 6）
  - [x] 子任务 4.1：形成最小人工验收清单
  - [x] 子任务 4.2：列出仍需人工补齐的 Linear / PR 外部动作
  - [x] 子任务 4.3：确保文档没有把缺失映射伪装成已完成事实
...
3. `docs/99_研发过程管理/13_EPIC-多Agent多Worktree并行开发/映射规则与PR必填字段.md`
4. `docs/99_研发过程管理/13_EPIC-多Agent多Worktree并行开发/验收/PAR-S5-验收检查单.md`
...
- 已形成 `映射规则与PR必填字段.md`，可直接作为各 Lane PR 描述模板和集成值班登记口径的输入。
- 已形成 `PAR-S5-验收检查单.md`，并按文档类 Story 采用人工验收口径完成收口。
- Linear 建单、真实 PR 编号和 TEA skills 安装仍为 Epic 级外部后续项，不阻塞本 Story 的文档交付完成。

### File List

- `docs/99_研发过程管理/13_EPIC-多Agent多Worktree并行开发/故事/PAR-S5-Linear-Story-PR-Worktree映射规则固化.md`
- `docs/99_研发过程管理/13_EPIC-多Agent多Worktree并行开发/映射规则与PR必填字段.md`
- `docs/99_研发过程管理/13_EPIC-多Agent多Worktree并行开发/验收/PAR-S5-验收检查单.md`
