...
2. Runtime 默认控制主链已经切到 `Go Runtime + Go Executor`，`RuntimeControlCore` 只保留 Python adapter 角色。
3. workpackage 当前一等公民入口仍是：
   - `entrypoint.py`
   - `entrypoint.sh`
4. `FactoryAgent + nanobot 适配层 + opencode builder` 仍是当前对外编排承接壳，正式状态机与 contract 已按 `EPIC15/16` 收口。
...
| 历史兼容背景 | `Redis / RQ` | 仅保留为历史实现说明，不再作为当前正式主骨架 |
...
2. Python 迁移 adapter 与历史 `Redis / RQ` 组织方式说明
...
5. [软件架构与技术选型分析报告](../99_研发过程管理/12_EPIC-软件架构与技术选型/分析报告.md)
