...
| Redis / RQ | 异步调度与分发层 | 队列、领取、重试、执行分发 | 充当正式结果真相源 |
...
| 服务 / 模块 | PostgreSQL | Redis / RQ | 对象 / 文件产物层 | 外部 LLM | 第三方 Provider |
|---|---|---|---|---|---|
...
| 可信数据管理模块 | 读写 | 可选 | 写快照/导入产物 | 不需要 | 需要 |
...
| Redis / RQ | 队列协议 | Runtime、Worker | 负责分发与重试 |
