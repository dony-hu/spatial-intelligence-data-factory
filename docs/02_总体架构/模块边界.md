...
| `runtime_orchestrator` | 执行面 | 任务调度、状态推进、执行触发 | Worker、`Go Runtime / temporal_go` 控制引擎、Runtime 记录 | CLI、页面、具体业务交互逻辑 |
| `governance_worker` | 执行面 | 装载工作包并回写执行结果 | Executor、Repository、Runtime 控制上下文 | 直接 import `address_core` 主线算法、`opencode` 推理调用 |
