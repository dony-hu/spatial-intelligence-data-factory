...
## 8. EPIC15 Agent 编排试点合同回归入口

当前针对 Agent 编排层框架替换试点，额外固定以下最小合同回归入口：

1. `tests/test_factory_agent_orchestration_core.py`
2. `tests/test_factory_agent_runtime_trace_logging.py`
3. `tests/test_factory_agent_routing.py`
4. `tests/test_factory_agent_contract.py`
5. `tests/test_orchestration_context_schema_contract.py`

统一执行脚本：

`scripts/run_epic15_contract_regression.py`

通过判定：

1. 可放行范围仅覆盖 Agent 编排层替换试点
2. 不覆盖 Runtime 控制层替换结论
3. 具体执行结果、报告与 warning 数应保留在对应 Epic 验收与 Ring3 测试产物中

## 9. EPIC16 模块语言切换回归入口

当前针对 `EPIC16` 模块语言切换，固定以下最小合同回归入口：

1. `tests/test_agent_runtime.py`
2. `tests/test_go_runtime_bridge.py`
3. `tests/test_runtime_control_core.py`
4. `tests/test_factory_agent_dryrun_contract.py`
5. `tests/test_factory_agent_dryrun_workflow.py`
6. `tests/test_factory_agent_publish_runtime.py`
7. `tests/test_factory_agent_observability_events.py`
8. `services/governance_api/tests/test_runtime_workpackage_real_chain_from_factory_agent.py`
9. `tests/test_factory_agent_publish_workflow.py`

通过判定：

1. `Go Runtime + Go Executor` 已进入默认主链
2. façade 合同仍保持稳定
3. bundle 入口 contract 与 truth-source 查询口径保持稳定
4. 具体 smoke 报告与回归结果应保留在 `EPIC16` 验收文件与 Ring3 `output/test-reports/` 中

`EPIC16-S5` 的最终通过口径额外要求：

1. `dryrun / publish` 回执必须带：
...
3. no-fallback 断言必须成立：
   - `runtime.compat.python_control_fallback_used = false`
4. PostgreSQL 真相源必须可回查：
   - `task_id`
   - `trace_id`
   - `workpackage_id@version`
   - `controller_impl`
   - `executor_impl`
5. `runtime_control_core.v1` 只能作为迁移前置基线，不得作为 `S5` 最终通过信号。
