...
| Runtime Orchestrator | 任务状态推进和调度 | 至少 1 个实例，需访问 Redis、PG |
| Governance Worker | 执行工作包 | 可多实例横向扩展 |
| PostgreSQL | 主真相源 | 主链路必须可备份和恢复 |
| Redis / RQ | 队列与执行分发 | 可重建，但丢失窗口需有补偿策略 |
...
2. PostgreSQL 和 Redis 是有状态组件，扩展前必须先确认备份、恢复和监控口径。
