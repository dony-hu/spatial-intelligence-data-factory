...
| create-epics-and-stories | `W-EPS` | `W-史诗拆分` | 拆分史诗与故事 |
...
- 在 Codex 对话中直接输入缩写：
  ...
  - `执行 A-ARC + W-ARC`
  - `执行 C-STS`
- 路由 Skill 负责把缩写映射到标准 BMAD skill 或 CLI 命令。

### 9.2 维护方式

- 新增缩写：编辑 `bmad-shortcuts-router/SKILL.md` 的映射表。
- 调整速查表：同步更新本文件，保持“文档口径”和“路由口径”一致。
- 升级 BMAD 后：抽查关键映射（`W-PRD`、`W-ARC`、`W-DEV`、`T-HLP`）是否仍存在于 `.agents/skills/`。
