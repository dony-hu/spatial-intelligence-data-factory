# Architecture - 系统可观测性能力构建（主线下一组 Story）

## 1. 文档信息

- 文档版本：v1.0
- 创建日期：2026-02-27
- 适用范围：地址治理主线下一阶段（可观测性能力）
- 关联文档：
  - `docs/architecture-address-governance-mvp-refresh-2026-02-27.md`
  - `docs/architecture-unified-pg-multi-schema-v1-2026-02-27.md`（PG 多 schema 强制主规范）
  - `docs/sprint-planning-spatial-intelligence-data-factory-2026-02-27.md`
...
4. 与现有 `/v1/governance/lab/observability/*` 能力兼容增量升级，不中断当前 MVP 交付链路。

## 3. 非目标（本阶段不做）

1. 不引入重型外部可观测平台（如独立 ELK/Tempo 集群）作为硬依赖。
2. 不做多租户隔离和细粒度 RBAC。
3. 不做全量历史数据重刷，只保证增量采集与关键窗口查询。
...
3. 存储层（Storage）
- 现有：`addr_audit_event`、`api_audit_log`、`addr_task_run`、`addr_workpackage_publish`。
- 新增（建议）：`addr_observation_event`、`addr_observation_metric`、`addr_alert_event`。
...
2. 告警流：聚合器判定阈值越界 -> 写 `addr_alert_event` -> 管理看板展示 -> 人工确认。
3. 回放流：按 `trace_id/task_id/workpackage_id` 查询事件序列 -> 重建时间线 -> 输出故障根因线索。

## 6. 数据模型设计

### 6.1 `addr_observation_event`（新增）
...
### 6.2 `addr_observation_metric`（新增）
...
1. OBS-1 统一事件模型与埋点 SDK
- 输出：统一事件结构、写库适配器、核心链路接入。

2. OBS-2 观测存储与查询 API
- 输出：新增观测表 + `snapshot/events/timeseries` API。

3. OBS-3 Trace 回放与故障定位
- 输出：`trace replay` 接口 + 页面时间线。

4. OBS-4 告警引擎与确认闭环
- 输出：告警规则执行、ack 接口、审计链路。

5. OBS-5 管理看板升级
- 输出：`lab/observability/view` 升级，接入新 API。

6. OBS-6 E2E 验收与门禁
- 输出：一键验收脚本、回归套件、DoD 证据索引。
...
1. 第 1 周：OBS-1 + OBS-2（先可采集、可查询）。
2. 第 2 周：OBS-3 + OBS-4（先可定位、可告警）。
3. 第 3 周：OBS-5 + OBS-6（先可视化、可验收）。
